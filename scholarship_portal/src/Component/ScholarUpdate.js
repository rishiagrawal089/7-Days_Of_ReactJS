...
function ScholarUpdate() {
  ...
  useEffect(()=>{
    fetch("http://localhost:8000/scholar/"+schId).then((res)=>{
      return res.json();
    }).then((resp)=>{
  ...
  const validatedForm = () => {
    if (
      ...
      (aadhaar < 100000000000 && aadhaar > 999999999999)
    ) {
      valChange(true);
    } else {
      valChange(false);
                  ...
                  <div className="col-lg-12">
                    <div className="form-group">
                      ...
                      <select
                        ...
                        className="form-control" value={branch}
                        required
                      >
